...
input_data='{\
 ...
 "data": {\
  ...
  "step1": {\
   ...
   "Cvc": "987",\
   "RespCode": "00",\
   "MsgOverride": [["049","A","008"],["004","A","000000001862"],["022","A","51010151310C001"],["041","A","66000002"],["042","A","42298501010101"],["048","A","8000"],["048.1","A","6699001500000500000"],["049","A","008"]]\
  }\
 }\
}'

def C1862_Sale(testrail_run_id):

    bpsim_out= Bpsim_0200.transaction_0200_POS(input_data)
    bpnode_transaction_history= Bpnode.transactions("62") #must be correct
    bpnode_event = Bpnode.events("currency") #no events

    if bpsim_out is None and (bpnode_event is None and '62' in bpnode_transaction_history):
